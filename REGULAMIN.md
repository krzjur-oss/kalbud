...
1. **Użytku domowego / prywatnego** — korzystanie przez osoby fizyczne w celach własnych, w tym projektowych, obliczeniowych, edukacyjnych i samokształceniowych.
...
2. Dane wprowadzane do Aplikacji (konfiguracje projektów, parametry opasek, podjazdów, parkingów, ogrodzeń, wykopów tarasów, zadaszeń drewnianych oraz cenniki materiałów) przechowywane są **wyłącznie lokalnie w pamięci przeglądarki użytkownika** (`localStorage`) i nigdy nie opuszczają jego urządzenia.
3. Administratorem danych osobowych wprowadzanych do programu (jeśli dotyczy) jest wyłącznie Użytkownik końcowy — Autor nie ma technicznego dostępu do tych danych.
4. Aplikacja nie używa śledzących plików cookie, zewnętrznych narzędzi analitycznych ani reklamowych systemów śledzenia.
5. Użytkownik może w każdej chwili usunąć swoje dane, czyszcząc dane przeglądarki lub korzystając z funkcji zarządzania projektami (usuwanie/reset projektów).
...
2. Wyceny i wyliczenia ilościowe materiałów generated przez Aplikację mają charakter pomocniczy i orientacyjny. Ostateczne zapotrzebowanie powinno być weryfikowane na budowie przez wykonawcę lub kierownika budowy.
3. Autor nie ponosi odpowiedzialności za utratę danych, błędy działania, awarie sprzętu lub szkody wynikające z korzystania bądź niemożności korzystania z Aplikacji.
